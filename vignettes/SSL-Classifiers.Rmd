...
line_coefficients_fromdual <- function(object) {
  stopifnot(is.null(object@scaling)) # Check the input has not been scaled
  w <- c(object@bias, object@Xtrain %*% object@alpha)
  list(intercept = (-(object@w[1])/object@w[3]), slope = (-object@w[2]/object@w[3]))
}
```

The RSSL package contains mostly methods for semi-supervised classification. This vignette serves as an overview of the available classifiers and their example outputs using a simple 2D datasets to illustate their behaviour.
...
* (Kernel) Least Squares Classifier
    ...
    * +(Contrastive Pessimistic Least Squares Classifier)
    * +(Imputed Least Squares Classifier)
    * Self-learning
...
df_to_matrices <- function(df,formula=NULL) {
  yu <- attr(df,"yu")
  if (!is.null(formula)) {
    df <- model.frame(formula,df,na.action=NULL)
  }
  d <- ncol(df)
  
  X <- model.matrix(~.-1, df[!is.na(df[,1]),2:d])
  Xu <- model.matrix(~.-1, df[is.na(df[,1]),2:d])
  y <- df[!is.na(df[,1]),1]
  
  return(list(X=X,Xu=Xu,y=y,yu=yu))
}
```
...
```{r echo=FALSE}
data_2gauss <- data.frame(generateGaussianClass(100,var=0.2)) %>% 
  add_missinglabels_mar(formula=Class~.,prob=0.9)
problem_2gauss <-  data_2gauss %>% df_to_matrices(Class~.)

problem1 <- problem_2gauss

p1 <- ggplot(data_2gauss,aes(x=V1,y=V2,shape=Class,color=Class)) +
...
```{r echo=FALSE}
data_slicedcookie <- data.frame(GenerateSlicedCookie(100))[sample(1:100),] %>% 
...
g_self <- SelfLearning(problem1$X,problem1$y,problem1$Xu,LeastSquaresClassifier)
```
...
g_self <- SelfLearning(problem2$X,problem2$y,problem2$Xu,LeastSquaresClassifier)
```
...
p2 + geom_abline(aes(intercept=intercept,slope=slope,linetype=Classifier),data=lines_ls,show_guide = TRUE)
```
...
g_emlda <- EMLinearDiscriminantClassifier(problem1$X,problem1$y,problem1$Xu)
g_sllda <- SelfLearning(problem1$X,problem1$y,problem1$Xu,LinearDiscriminantClassifier)
```

```{r echo=FALSE, warning=FALSE, fig.width=8, fig.height=5, dev="svg"}
lines_lda <- data_frame(
  intercept=c(line_coefficients(g_lda)$intercept, 
              line_coefficients(g_emlda)$intercept,
              line_coefficients(g_sllda@model)$intercept
              ),
  slope=c(line_coefficients(g_lda)$slope,
          line_coefficients(g_emlda)$slope,
          line_coefficients(g_sllda@model)$slope
          ),
  Classifier=factor(c("LDA","EMLDA","SLLDA"),levels=c("LDA","EMLDA","SLLDA"),ordered=TRUE)
)

p1 +  geom_abline(aes(intercept=intercept,slope=slope,linetype=Classifier),data=lines_lda,show_guide = TRUE)
```

```{r}
g_lda <- LinearDiscriminantClassifier(problem2$X,problem2$y)
g_emlda <- EMLinearDiscriminantClassifier(problem2$X,problem2$y,problem2$Xu)
g_sllda <- SelfLearning(problem2$X,problem2$y,problem2$Xu,LinearDiscriminantClassifier)
```
...
p2 +  geom_abline(aes(intercept=intercept,slope=slope,linetype=Classifier),data=lines_lda,show_guide = TRUE)
```
...
## Semi-Supervised Learning based on Low density separation
...
```{r}
g_svm <- SVM(problem1$X,problem1$y)
g_tsvm <- TSVM_CCCPlin(problem1$X,problem1$y,problem1$Xu,C=1,Cstar=1)
```
...
```{r}
g_s4vm <- S4VM(problem1$X,problem1$y,problem1$Xu) # Returns NULL
```

### Entropy Regularization (Logistic Regression)
```{r}
  g_lr <- LogisticLossClassifier(problem2$X,problem2$y)
  g_erlr <- ERLogisticLossClassifier(problem2$X,problem2$y,problem2$Xu,lambda_entropy = 0,init = rnorm(3))
```
...
generateTwoCircles(200,0.05) %>%
  ggplot(aes(X.1,X.2,color=Class)) %>%
  + geom_point() %>%
  + coord_equal()

data <- generateTwoCircles(200, 0.05) 
Xin <- as.matrix(data[,1:2])
yin <- model.matrix(~Class-1,data)[1:2,1,drop=FALSE]
...
g_proj_sup <- ICLeastSquaresClassifier(problem1$X,problem1$y,problem1$Xu,projection = "supervised")
g_proj_semi <- ICLeastSquaresClassifier(problem1$X,problem1$y,problem1$Xu,projection = "semisupervised")
g_proj_euc <- ICLeastSquaresClassifier(problem1$X,problem1$y,problem1$Xu,projection = "euclidean")
```
...
g_semi <- ICLinearDiscriminantClassifier(problem1$X,problem1$y,problem1$Xu)
```
...
### Testing improvements in the loss

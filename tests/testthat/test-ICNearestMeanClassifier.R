context("ICNearestMeanClassifier")
...
test_that("Formula and matrix formulation give same results",{
  g_matrix <- ICNearestMeanClassifier(X,y,X_u)
  g_model <- ICNearestMeanClassifier(modelform,D)
  expect_equal(predict(g_matrix,X_test), predict(g_model,D_test))
  expect_equal(loss(g_matrix, X_test, y_test),loss(g_model, D_test),tolerance=10e-5)
})
